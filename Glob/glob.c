...
int lahf(glob_t *glob, char *buf, unsigned long size) {
    ...
    sprintf(glob->registers->ax, "%d%d%d%d%d", glob->flags->sf, glob->flags->zf,
            glob->flags->af, glob->flags->pf, glob->flags->cf);
...
char *opptr(glob_t *glob, char *operand) {
    ...
    return (node)
               ? node->val
               : register_addr(glob, SEG_INIT_ADDR, (int)strtol(addr, NULL, 0))
                     ->val;
}
...
ab_lit:;
    ...
    if (*lchar == 'H' || *lchar == 'h' || size == -1) {
        ...
        if (val > 32767 || val < -32768) {
            fprintf(stderr, "opval(): Operand value too large [%s].\n",
                    operand);
            glob->flags->of = 1;
            return 1;
        }
